...
internal class ReplayManager
{
    ...
    public ReplayManager(GameInstance game, string replayPath, Logger log)
    {
        this.game = game;
    ...
    {
        this.log.Verbose("Replay - saving lobby data...");
        ...
        this.log.Verbose("Replay - lobby data saved.");

        this.replayWriter?.Flush();
        this.log.Verbose("Replay - lobby data flushed.");
    }
    ...
    public void AddGameState(int tick, string gameStateId)
    {
        ...
        this.log.Verbose("Replay - adding game state ({tick})...", tick);
        ...
        this.log.Verbose("Replay - game state added ({tick}).", tick);

        if (tick % FlushIntervalTicks == 0)
        {
            this.replayWriter.Flush();
            this.log.Verbose("Replay - game state flushed ({tick}).", tick);
        }
    }
    ...
    {
        this.log.Verbose("Replay - saving game end...");
        ...
        this.log.Verbose("Replay - game end saved.");

        this.replayWriter?.Flush();
        this.log.Verbose("Replay - game end flushed.");
    }
    ...
    public void SaveReplay()
    {
        ...
        {
            this.log.Warning("Replay stream not initialized. Replay will not be saved.");
            return;
        }

        if (this.gameEnd is null)
        {
            this.log.Warning("Saving replay with missing game end.");
        }
        ...
        try
        {
            ...
            this.log.Debug("Replay written to temp file: {path}", this.tempJsonPath);

            if (this.replayPath.EndsWith(".zip", StringComparison.OrdinalIgnoreCase))
            {
                this.log.Debug("Creating zip archive...");
            ...
            {
                this.log.Debug("Creating tar.gz archive...");
        ...
        {
            this.log.Error(ex, "Saving replay failed.");
            return;
        }

        this.log.Information("Replay saved to: {replayPath}", this.replayPath);
    }
    ...
    public void SaveResults()
    {
        ...
        try
        {
            ...
            this.log.Debug("Saving results to: {savePath}", savePath);
            File.WriteAllText(savePath, results.ToString());
        }
        catch (Exception ex)
        {
            this.log.Error(ex, "Saving results failed.");
            return;
        }

        this.log.Information("Results saved to: {savePath}", savePath);
    }
